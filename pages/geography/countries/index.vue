<template>
  <div>
    <PageHeader :breadcrumbs="breadcrumbs" title="Manage Countries">
    </PageHeader>
    ...
    >
      <template #header>
        Total {{ collection ? collection['hydra:totalItems'] : 0 }} Countries
      </template>
    </DataTable>
...
import { createCountry } from '~~/types/entity/country';
...
const filters = useState('countries.filters', () => ({}));
const countryService = new CountryService();

const formLoading = ref(false);
const formVisible = ref(false);
const formState = ref(createCountry());
...
const {
  ...
  'countries',
  () => countryService.collection(filters.value),
  {
    watch: [filters],
...
const submitForm = async (isFormValid) => {
  ...
  try {
    await countryService[method](formState.value);
