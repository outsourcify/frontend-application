...
import { Affiliate } from '~~/types/entity/affiliate';
...
const destinationService = new AffiliateService();
const {
  data: collection,
  pending: loading,
  error,
...
const options = computed<Affiliate[]>(() => {
  if (!collection.value) {
    return [];
  }

  return _.map(collection.value['hydra:member'], (item: Affiliate) => {
    return {
      label: `${item.name}`,
      value: item['@id'],
    };
